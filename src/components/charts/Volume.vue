  ...
  get tooltip() {
    return {
      ...
      formatter: ([params]) => {
        ...
        return [
          `<div class="volume-chart__tooltip">`,
          `<div class="volume-chart__tooltip-value">${value}</div>`,
          `<div class="volume-chart__tooltip-time">${time}</div>`,
          `</div>`,
        ].join("");
      },
      position: function () {
...
.volume-chart {
  ...
  ::v-deep {
    .volume-chart__tooltip {
      .volume-chart__tooltip-value {
        color: var(--v-primary-base);
        font-size: 18px;
        margin-bottom: 4px;
      }

      .volume-chart__tooltip-time {
        font-size: 12px;
      }
    }
  }
